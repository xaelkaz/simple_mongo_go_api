...
func (db *DB) GetMovies(w http.ResponseWriter, r *http.Request) {
	var movies []Movie
	cur, err := db.collection.Find(context.TODO(), bson.M{})
	...
	if err != nil {
		...
		for cur.Next(context.TODO()) {
			...
			err := cur.Decode(&movie) // decode similar to deserialize process.
			if err != nil {
				log.Fatal(err)
			}
...
func main() {
	...
	r.HandleFunc("/v1/movies", db.GetMovies).Methods("GET")
